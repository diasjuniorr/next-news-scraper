...
// const website = process.env.ECONOMIST_URL || "";
  ...
  return new Promise<void>((resolve, reject) => {
    try {
      axios(website).then((response) => {
        ...
        $("a", data).each((idx, el) => {
          ...
          if (typeof attr !== typeof undefined) {
            if ($(el).attr("data-analytics")?.includes("top_stories")) {
              ...
              const img =
                $(el).parent().siblings().find("img").attr("src") || "";
        ...
        res.status(200).json({ headlines, discovers, favorites, sections });
        resolve();
      });
...
function isUnauthorizedUrl(url: string) {
  return url.includes("econ.st");
}
